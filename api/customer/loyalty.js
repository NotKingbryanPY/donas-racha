...
module.exports = withApi(['GET'], async req => {
  ...
  const [accounts, streaks, badges, transactions] = await Promise.all([
    serviceRequest('loyalty_accounts', { query: new URLSearchParams({ select: 'available_points,lifetime_points,purchase_points,level_key,version,updated_at,loyalty_levels(name,emoji,minimum_lifetime_points)', customer_id: `eq.${customer.id}`, limit: '1' }).toString() }),
