...
(async()=>{
 ...
 for (const width of [360,390,412,768,1440]) {
  ...
  await page.route('**/*',async route=>{
   ...
   if(u.hostname==='script.google.com') {
    const data=req.method()==='POST'?JSON.parse(req.postData()):Object.fromEntries(u.searchParams); requests.push(data);
    let response={ok:true};
    switch(data.action){
     case 'verificarPassword':response={ok:data.password==='demo'};break;
     case 'getRanking':response={ok:true,ranking:[client,{...client,id:'C2',name:'Segundo cliente'},{...client,id:'C3',name:'Tercer cliente'}]};break;
     case 'buscarCliente':response={ok:true,clients:data.q==='nadie'?[]:[client]};break;
     case 'getCliente':response=data.id==='INVALID'?{ok:false,error:'Cliente no encontrado'}:{ok:true,client};break;
  ...
  await page.locator('#loginInput').fill('demo');await page.locator('#loginBtn').click();await page.waitForFunction(()=>sessionStorage.getItem('donasAuth')==='true');
  ...
  await page.locator('.result-item').focus();await page.keyboard.press('Enter');await page.waitForSelector('#buyBtn:visible');
  ...
  const saved=requests.find(r=>r.action==='saveConfig');assert(saved);assert.equal(saved.adminPassword,'demo');assert.equal(saved.config.PRECIO_DONA,'1');
  await page.evaluate(()=>logout());assert.equal(await page.evaluate(()=>sessionStorage.getItem('donasAuth')),null);
  await page.evaluate(()=>goToUserLogin());await page.locator('#userScanBtn').click();await page.waitForFunction(()=>userScannerOpen);
  await page.evaluate(()=>window.__scanners.at(-1).success('C-DEMO'));await page.waitForFunction(()=>document.getElementById('clientName').textContent.includes('Cliente de prueba'));
 ...
 await page.route('https://**/*',r=>r.fulfill({body:r.request().url().includes('script.google')?'{}':'',contentType:r.request().url().includes('script.google')?'application/json':'text/javascript'}));
