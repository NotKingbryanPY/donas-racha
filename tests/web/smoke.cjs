...
(async () => {
  ...
  await context.route('**/api/**', async route => {
    ...
    else if(path==='/api/customer/session') data={accessToken:'test-customer-token',expiresAt:new Date(Date.now()+3600000).toISOString()};
  ...
  await catalog.getByText('7 restantes').waitFor({timeout:10000});
  assert.equal(errors.length,0,errors.join('\n'));
  console.log('PASS web smoke: admin stock/mode, customer points/QR, catalog remaining');
  await browser.close();
