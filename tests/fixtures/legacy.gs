...
const SYSTEM_VERSION = '2.1.0-tienda';
...
function ensureSheet_(ss, name, headers, color) {
  ...
  if (sh.getLastRow() === 0 || sh.getLastColumn() === 0) {
    ...
    let existing = sh.getRange(1, 1, 1, lastCol).getValues()[0].map(h => String(h || '').trim());
...
function keyRowMap_(sh, keyHeader) {
  ...
  const data = sh.getRange(2, keyCol, sh.getLastRow() - 1, 1).getValues();
...
function findClientById_(id) {
  ...
  for (let i = 0; i < rows.length; i++) {
    if (String(get_(rows[i], map, 'ID')) === String(id)) return { sheet:sh, map:map, row:rows[i], rowNum:i+2 };
  }
  return null;
}
